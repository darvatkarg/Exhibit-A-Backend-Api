...
export const get_profile = async (req: Request, res: Response) => {
    reqInfo(req)
    try {
        let response = await userModel.findOne({ _id: ObjectId((req.header('user') as any)?._id), isActive: true, userType: userStatus.teacher }, { _id: 1, name: 1, email: 1, phoneNumber: 1, alterPhoneNumber: 1, image: 1, schoolId: 1 })
